...
public static class ItemEndpoints
{
    public static RouteGroupBuilder MapItemEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPut("/{id:int}", UpdateItemAsync)
            .Produces<ItemDto>(200)
            .ProducesProblem(400)
            .Produces(404)
            .ProducesProblem(401)
            .ProducesProblem(409)
            .WithTags("Items")
            .WithDescription("编辑物品（名称/位置/标签/备注）");
    ...
    private static async Task<Results<Ok<ItemDto>, NotFound, ProblemHttpResult>>
        UpdateItemAsync(int id, UpdateItemRequest request, ItemRepository repo,
            LocationRepository locationRepo, UserManager<AppUser> userManager,
            HttpContext httpContext, CancellationToken ct)
    {
        try
        {
            ...
            var item = await repo.UpdateAsync(id, request.Name, request.LocationId,
                request.TagIds, request.Note, userId, ct);
            if (item is null) return TypedResults.NotFound();
        ...
        {
            return TypedResults.Problem("物品已被他人修改，请刷新后重试", statusCode: 409);
        }
        catch (ArgumentException ex)
        {
            return TypedResults.Problem(ex.Message, statusCode: 400);
